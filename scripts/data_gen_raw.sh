...
SYNC_LOOP_COUNT=8  # Set this to the number of parallel processes you want
ASYNC_LOOP_COUNT=5   # Number of asynchronous runs per sync run


for ((i=0; i<SYNC_LOOP_COUNT; i++)); do
    for ((j=0; j<ASYNC_LOOP_COUNT; j++)); do
        python -m embodichain.lab.scripts.run_env \
            --gym_config gym_config_raw.json \
            --headless --enable_rt --num_envs 20 --gpu_id 1 &
        sleep 5
